...
-- common helpers are provided by Base
...
function M.finish_suite(result)
  local errs = Base.count_errors(result.benchmarks)
  if errs > 0 then
    return "  "
      .. green
...
-- rely on Base.format for common scaffolding

function M.single_header(result)
  local parts = {
    "",
    blue
      .. fmt("  runtime %s  ", result.runtime_version or "unknown")
...
-- ensure old-style call `M.format(result)` works
M.format = function(result)
  return Base.format(M, result)
end
