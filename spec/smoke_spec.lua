...
describe("chrono.format", function()
  ...
  describe("csv", function()
    it("includes a header and one row per benchmark", function()
      local s = chrono.suite("cfmt", { iterations = 5 })
      s:add("a", function() end)
      s:add("b", function() end)
      local csv = chrono.format(s:run(), "csv")
      assert.is_truthy(csv:find "suite_name,benchmark_index,name,status")
      assert.is_truthy(csv:find "cfmt,1,a,ok")
      assert.is_truthy(csv:find "cfmt,2,b,ok")
    end)
  end)
